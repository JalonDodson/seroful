...
import { userState, loginHelperState, registerHelperState } from "../../store/store";
  ...
  useEffect(() => {
    if (loginHelper.errorMsg) {
      if (loginHelper.errorMsg === "auth/invalid-email") {
        setLoginHelper(
          (x) => (x = { ...x, txt: "Oh noes, that's an invalid email!", emailError: true, })
        );
  ...
  useEffect(() => {
    if (registerHelper.errorMsg) {
      if (registerHelper.errorMsg === "auth/email-already-in-use") {
        setRegisterHelper(
          (x) => (x = { ...x, txt: "Aw snap, that email is already in use! Did you mean to login?", emailError: true, })
        );
        ...
        <div className={styles.loginDiv}>
          ...
          <form noValidate autoComplete="off">
            <TextField
              ...
              onFocus={() =>
                setLoginHelper((x) => (x = { ...x, emailError: false, txt: "" }))
              }
              onChange={(ev) =>
        ...
        <div className={styles.loginDiv}>
          ...
          <form noValidate autoComplete="off">
            <TextField
              ...
              onFocus={() =>
                setRegisterHelper((x) => (x = { ...x, emailError: false, txt: "" }))
              }
              label="Email"
              variant="filled"
              className={styles.email}
            ...
            <TextField
              ...
              value={pwConfirm}
              className={styles.password}
              onBlur={() => setPwHelpers(x => x = { helperText: "", error: false })}
